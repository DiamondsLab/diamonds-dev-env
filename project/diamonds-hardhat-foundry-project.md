# Diamonds Hardhat Foundry Project

We are working on a module called `@diamondslab/diamonds-hardhat-foundry` located in the `packages/diamonds-hardhat-foundry` directory which is part of the Diamonds Dev Env monorepo project. The Diamonds Hardhat Foundry module provides integration between Hardhat and Foundry for testing and deploying Diamond contracts using the Diamonds module. The aboslute path for the monorepo is `/worskpace/diamonds-dev-env`. The path for the Diamonds Hardhat Foundry module is `/workspace/diamonds-dev-env/packages/diamonds-hardhat-foundry`.

The purpose of the `@diamondslab/diamonds-hardhat-foundry` is to provide Foundry Forge helpers, functionality and tools that interact with and enable the typescript node `@diamondslab/diamonds` module found in `packages/diamonds` directory and the typescript node module `@diamondslab/hardhat-diamonds` which is found under `packages/hardhat-diamonds` directory, when sing foundry tools for testing, deploying and other cases in a hardaht project that uses the diamonds module.

The main focus of our `@diamondslabs/diamonds-hardhat-foundry` module is utilize Foundry Forge for unit, integration, fuzz and invariant testing smart contracts that use our hardhat based erc-2535 diamond proxy contract management system we call `Diamonds` (`@diamondslab/diamonds`).

We can see how this works with Hardhat already by looking at `test/deployments/DiamondDeployment.test.ts`. Test are launched to a local node/fork that is running (or made to run with the test runner) with either hardhat or anvil using the functionality already built in to the `diamonds` module and `@diamondslab/hardhat-diamonds` module. The DiamondCutFacet, DiamondLoupeFacet, Diamond and all the other facets are deployed (or upgraded) using the LocalDiamondDeployer. THe LocalDiamondDeployer utilizes the Diamonds module (`@diamondslab/diamonds`) with the submission of a configuration `LocalDiamondDeployerConfig` (config) to `LocalDiamondDeployer.getInstance()` along with the HardhatRuntimeEnvironment (hre).

Creating a `Diamond` object with `LocalDiamondDeployer.getDiamondDeployed()` method also deploys or upgrades the Diamond and Facets, registers the Function Selectors and runs the initializer function using the DiamondCut process. This is all accomplished within the Diamonds Module.

We can then access the deployed contract addresses and deployer address from the `Diamond` object using `diamond.getDeployedDiamondData()`.
and use these for running our tests.

The binary choice between the deployment of new diamond in total or the upgrade of an existing diamond relies on the existence of a filled deployment record. We can choose to write the deployment record as part of our deployment process by setting writeDeployeDiamondData switch in the `LocalDiamondDeployerConfig`

## Project Monorepo Overview: Diamonds Dev Env
