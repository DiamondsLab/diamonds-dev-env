	...
	try {
		...
		const checkStatus = async (): Promise<void> => {
			...
			try {
				...
				console.log(
					`Deployment Status: ${getStatusEmoji(deploymentStatus)} ${deploymentStatus}`,
				);
				...
				if (deployedDiamondData.DiamondAddress) {
					...
					try {
						...
						console.warn(
							`❌ Failed to check on-chain status: ${error instanceof Error ? error.message : error}`,
						);
					}
				} else {
				...
				if (deployedDiamondData.DeployedFacets) {
					...
					for (const [facetName, facetInfo] of Object.entries(
						...
						console.log(
							`  ${facetName}: ${facetInfo.address} (v${facetInfo.version || '0.0'})`,
						);
					}
				}
...
function showUsage(): void {
	console.log(
		'Usage: npx ts-node scripts/deploy/status-defender.ts [DIAMOND_NAME] [NETWORK_NAME] [PROPOSAL_ID]',
	);
	console.log('');
	...
	console.log(
		'  npx ts-node scripts/deploy/status-defender.ts GeniusDiamond polygon abc123 --watch --interval=30',
	);
}
