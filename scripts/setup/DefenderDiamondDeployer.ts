...
export class DefenderDiamondDeployer {
	...
	private async initialize(): Promise<void> {
		try {
			...
			if (this.verbose) {
				console.log(
					`✅ DefenderDiamondDeployer initialized for ${this.config.diamondName} on ${this.config.networkName}`,
				);
			}
		} catch (error) {
			throw new Error(
				`Failed to initialize DefenderDiamondDeployer: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
		}
	}
	...
	private async setupProviderAndSigner(): Promise<void> {
		...
		if (this.verbose) {
			console.log(
				`🔗 Connected to ${this.config.networkName} (Chain ID: ${this.config.chainId})`,
			);
	...
	public async deployDiamond(): Promise<Diamond> {
		try {
			...
			throw new Error(
				`Diamond deployment failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
		}
	}
	...
	private async generateDiamondAbi(): Promise<void> {
		try {
			...
			console.warn(
				`⚠️ Failed to generate Diamond ABI: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
	...
	public static loadNetworkConfig(networkName: string): NetworkConfig {
		...
		try {
			...
			throw new Error(
				`Failed to load network configuration: ${error instanceof Error ? error.message : 'Unknown error'}`,
			);
		}
	}
